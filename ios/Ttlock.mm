...
{
    return std::make_shared<facebook::react::NativeTtlockSpecJSI>(params);
}
...
RCT_EXPORT_METHOD(controlLock:(NSInteger)controlAction lockData:(NSString *)lockData success:(RCTResponseSenderBlock)success fail:(RCTResponseSenderBlock)fail)
{
    ...
    TTControlAction action = TTControlAction (controlAction + 1);
...
{
    TTOperateLogType logType = TTOperateLogType(type + 1);
...
{
    TTLockConfigType type = TTLockConfigType (config + 1);
...
{
    [TTLock setLockSoundWithSoundVolume:(TTSoundVolume)soundVolume lockData:lockData success:^{
        [Ttlock reseponseSuccess:nil success:success];
    } failure:^(TTError errorCode, NSString *errorMsg) {
        [Ttlock responseFail:LOCK code:errorCode errorMessage:errorMsg fail:fail];
    }];
...
{
    [TTLock setUnlockDirection:(TTUnlockDirection)unlockDirection lockData:lockData success:^{
        [Ttlock reseponseSuccess:nil success:success];
    } failure:^(TTError errorCode, NSString *errorMsg) {
        [Ttlock responseFail:LOCK code:errorCode errorMessage:errorMsg fail:fail];
    }];
...
{
    TTPassageModeType modeType = TTPassageModeType(type + 1);
...
{
    [TTLock recoverPasscode:passcode newPasscode:passcode passcodeType:(TTPasscodeType)passcodeType startDate:startDate.longLongValue endDate:endDate.longLongValue cycleType:cycleType lockData:lockData success:^{
        [Ttlock reseponseSuccess:nil success:success];
    } failure:^(TTError errorCode, NSString *errorMsg) {
        [Ttlock responseFail:LOCK code:errorCode errorMessage:errorMsg fail:fail];
    }];
...
RCT_EXPORT_METHOD(setLiftWorkMode:(int) workMode lockData:(NSString *)lockData success:(RCTResponseSenderBlock)success fail:(RCTResponseSenderBlock)fail)
{
  ...
  [TTLock setLiftWorkMode:(TTLiftWorkMode)workMode lockData:lockData success:^{
    [Ttlock reseponseSuccess:nil success:success];
  } failure:^(TTError errorCode, NSString *errorMsg) {
    [Ttlock responseFail:LOCK code:errorCode errorMessage:errorMsg fail:fail];
  }];
...
RCT_EXPORT_METHOD(initGateway:(NSDictionary *)dict success:(RCTResponseSenderBlock)success fail:(RCTResponseSenderBlock)fail)
{
    ...
    TTGatewayType gatewayType = (TTGatewayType)[dict[@"type"] intValue];
...
{
    [TTLock getAccessoryElectricQuantityWithType:(TTAccessoryType)type accessoryMac:mac lockData:lockData success:^(NSInteger electricQuantity, long long updateDate) {
...
+ (void)responseFail:(DEVICE)device code:(NSInteger)code errorMessage:(NSString *)errorMessage fail:(RCTResponseSenderBlock)fail{
  ...
  if(device == GATEWAY){
      errorCode = @([self getTTGatewayErrorCode:(TTGatewayStatus)code]);
      ...
      errorCode = @([self getTTDoorSensoryErrorCode:(TTDoorSensorError)code]);
  }else{
